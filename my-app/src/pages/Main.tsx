...
function Main() {
    ...
    function swipeLeft() {
        setCurrentProduct((prevProduct) => {
            if(currentProduct === 0){
                return favoritesProducts.length - 1;
            } else {
                return prevProduct === 0 ? favoritesProducts.length - 1 : prevProduct - 1;
            }
        });
    }
    ...
    function swipeRight(){
        setCurrentProduct((prevProduct) => {
            if(currentProduct === favoritesProducts.length - 1){
                return 0;
            } else {
                return (prevProduct === favoritesProducts.length - 1) ? 0 : prevProduct + 1;
            }
        });
    }
    ...
    return <>
        <div className="header-main">
            ...
            <div className="header-text">
                <h1><span style={{"color": "rgba(176, 144, 122, 1)", "fontStyle": "italic"}}>{t("main_page_title_enjoy")}</span> {t("main_page_title_premium")}</h1>
                <p>{t("hook_sentence")}</p>
    ...
    <main id="content">
        <section id="favorite-coffee">
            <h2>{t("favorite_coffee_title_choose")} <span style={{"color": "rgba(176, 144, 122, 1)", "fontStyle": "italic"}}>{t("favorite_coffee_title_favorite")}</span> {t("favorite_coffee_title_coffee")}</h2>
            <div className="coffee-cards">
                ...
                <div className="coffee-card">
                    {favoritesProducts.map((p) => (
                        <div className="product-card fade" key={p.id} style={{display: (favoritesProducts.indexOf(p) === currentProduct) ? "block" : "none"}}>
                            <img src={`${coffeeImages[p.name]}`} alt="product-img" />
                            <h3>{t(p.name)}</h3>
                            <p>{t(`${p.name} description`)}</p>
                            <h3>${p.price}</h3>
                        </div>
                    ))}
                </div>
            ...
            <div className="coffee-slider">
                <div className="sliders">
                    {
                        favoritesProducts.map((_, index) => {
                            return <span key={index}>
                                <svg width="40" height="4" viewBox="0 0 40 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <rect width="40" height="4" rx="2" fill={index === currentProduct ? "#665F55" : "#C1B6AD"}/>
                                </svg>
                            </span>
                        })
                    }
        ...
        <section id="about">
            <h2>{t("resource_title_resource")} <span style={{color: "rgba(176, 144, 122, 1)", fontStyle: "italic"}}>{t("resource_title_perfect_and_cozy")}</span> {t("resource_title_where")}</h2>
        ...
        <section id="mobile-app">
            <div className="mobile-app-text">
                <h2><span style={{color: "rgba(176, 144, 122, 1)", fontStyle: "italic"}}>{t("download_title_download")}</span> {t("download_title_our_apps")}</h2>
                ...
                <div className="download-buttons">
                    <button className="app-store">
                        ...
                        <span className="app-store-text">
                            <span>{t("download_button_available")} </span>
